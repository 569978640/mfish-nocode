...
public class GatewayAssistant {
    ...
    public GatewayAssistant(ChatModel openAiChatModel, ChatMemory chatMemory) {
        ...
        this.chatClient = ChatClient.builder(openAiChatModel)
                .defaultSystem("""
                        你是"WindFlow"的路由小助手，你会根据用户的问题，判断用户需要访问的路由路径!
                        当有人问"WindFlow"相关信息时，实际是在问我们整个平台的信息
                        你只能返回路由路径，不能返回其他内容！
                        结果以json格式返回：
                        返回格式：{"path":"/sys/ai/chat","name":"WindFlow小助手"}

                        # 路由路径
                        1. /sys/ai/chat : WindFlow小助手 默认路由
